...
class ModelWorker:
    def __init__(self, model_path='tencent/Hunyuan3D-2', device='cuda'):
        ...
        self.i23d_worker = Hunyuan3DDiTFlowMatchingPipeline.from_pretrained(model_path, device="cpu")
        self.t2i_worker = HunyuanDiTPipeline('Tencent-Hunyuan/HunyuanDiT-v1.1-Diffusers-Distilled',
                                               device='cpu')
        self.texgen_worker = Hunyuan3DPaintPipeline.from_pretrained(model_path)
        self.pipeline = offload.extract_models("i23d_worker", self.i23d_worker)
        self.pipeline.update(  offload.extract_models( "texgen_worker", self.texgen_worker))
        self.texgen_worker.models["multiview_model"].pipeline.vae.use_slicing = True
        self.pipeline.update(offload.extract_models( "t2i_worker", self.t2i_worker))

        if profile < 5:
            kwargs["pinnedMemory"] = "i23d_worker/model"
        if profile !=1 and profile !=3:
            kwargs["budgets"] = { "*" : 2200 }
        offload.profile(self.pipeline, profile_no = profile, verboseLevel = int(args.verbose), **kwargs)
...
if __name__ == "__main__":
    ...
    worker = ModelWorker(model_path=args.model_path, device='cpu')
